...
- **骨架式角色动画**：关节分层的角色模型，胸鳍、尾鳍、眼睛、身体各自独立运动，内置挥手、点头、摇头、蹦跳、摆尾、打瞌睡等 10 种动作
- **说话配动作**：每句语料都绑定了动作，说到「一起去看海吧？」会摆尾，说到「呜哇！吓到了吗？」会跳一下；平时待机也会每隔几秒自己动一动
...
| 版本 | 文件 | 大小 | SHA-256 |
|---|---|---|---|
| v1.4 | [balance-bubble-1.4.apk](https://github.com/kekejiang114514/token-balance-bubble/releases/download/v1.4/balance-bubble-1.4.apk) | 480 KB | `dc3e8240…fb18` |
...
# dc3e824056a05a1f64cbe10e6cda98c63bdc9fdc9b4709347a0cd96f18cffb18
```
...
**角色的动作**在设置页「③ 模式与显示」里有独立开关，关掉就静态站立。角色本身是骨架式模型，不依赖图片素材：胸鳍绕肩点旋转（挥手）、尾鳍摆动（游动）、眼睛开合（眨眼、打瞌睡）、身体缩放位移（呼吸、蹦跳、落地挤压）。
...
app/
  src/com/coco/balancebubble/
    ...
    PetView.java         骨架式角色：部件绘制、关节变换、动作播放
  ...
  assets/char.png        角色贴图（v1.4 起改用骨架角色，此图暂未使用）
  res/mipmap/            应用图标
tools/                   构建与校验脚本
...
- **v1.4** —— 角色改成骨架式动画（挥手、眨眼、摆尾等 10 种动作，说话配动作）；气泡宽度跟着字数走、长句自动折行；模式细分为仅 token 查询 / 仅桌宠 / 混合模式，混合模式会在余额变化时主动提醒
...
- v1.4 起角色改为代码绘制的骨架模型，`assets/char.png` 暂时没有再被使用（保留在包里）
