...
- **局部形变**：在整体姿态之上再叠一层 26×26 网格形变，单独驱动脸、发丝和双手——每 2.2~6 秒眨一次眼、头顶与两侧长发随风漂移、甩尾时两条长发朝同一侧摆开、挥手时右臂绕腕点转动；脸和手是保护区，不会被发丝拖走
...
```sh
sha256sum balance-bubble-1.5.apk
# bf9066b6946979567ac6ae0c84e3e59e7c0520672130e39e829c2a0bb33f09f0
```

**版本关系**：六个版本的包名和签名相同，可以直接覆盖安装，设置和 API Key 都会保留。如果只想用最新版，装 v1.5 就行。
...
在这层整体姿态之上，还有一层**局部形变**：贴图被切成 26×26 的网格，用几组权重场分别驱动脸、发丝和双手。于是不用把立绘拆成零件，也能做出眨眼（每 2.2~6 秒一次，眼睛纵向压下去）、发丝随风（头顶和两侧长发按两条不同相位的正弦漂移）、甩尾（两条长发朝同一侧摆开）、挥手（右臂绕腕点转动，左手只跟着晃一点）。脸和手是保护区，权重场在那里被压到 0，发丝摆动不会把脸一起拖走。
...
app/
  src/com/coco/balancebubble/
    ...
    PartRig.java         局部形变：26×26 网格 + 权重场（眨眼/发丝/甩尾/挥手，纯逻辑可单测）
...
> 说明：v1.0 和 v1.1 只保留了签名后的 APK，源码快照没有留存，仓库中的 `app/src` 是 v1.5 的代码。
